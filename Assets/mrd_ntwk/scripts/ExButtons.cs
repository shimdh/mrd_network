...
//        right side
				...
				if (GUI.Button (new Rect(160, 505, 130, 45), "Set Own Costumes")) {
						SetOwnCostumes ();
				}

				if (GUI.Button (new Rect(160, 560, 130, 45), "Get Own Costumes")) {
						GetOwnCostumes ();
				}

				// Another side
				if (GUI.Button (new Rect(310, 10, 130, 45), "Set Own Costume Bases")) {
						SetOwnCostumeBases ();
				}

				if (GUI.Button (new Rect(310, 65, 130, 45), "Get Own Costume Bases")) {
						GetOwnCostumeBases ();
				}
		...
		void SetOwnCostumes ()
		{
				...
				costumes.Add ("COS_C01");
				costumes.Add ("COS_C02");
				costumes.Add ("COS_C03");				
				

				var dict = new Dictionary<string, object> ();
				dict.Add ("session_id", mrdPrt.sessionId);
				dict.Add ("own_costumes", costumes);

				mrdPrt.Connecting (dict, Enum.GetName (typeof(ProtocolType), ProtocolType.set_owncostumes));
		}
		...
		void GetOwnCostumes ()
		{
				...
				mrdPrt.Connecting (dict, Enum.GetName (typeof(ProtocolType), ProtocolType.get_owncostumes));
		}

		void SetOwnCostumeBases ()
		{
				var costumebase_list = new List<Dictionary<string, object>> ();

				var got_costumebase = new Dictionary<string, object> ();
				got_costumebase.Add ("costumebase_index", "COB_C01");
				got_costumebase.Add ("lastdate_from_gotcash", "2013,06,25");

				costumebase_list.Add (got_costumebase);

				var dict = new Dictionary<string, object> ();
				dict.Add ("session_id", mrdPrt.sessionId);
				dict.Add ("own_costumebases", costumebase_list);

				mrdPrt.Connecting (dict, Enum.GetName (typeof(ProtocolType), ProtocolType.set_owncostumebases));
		}

		void GetOwnCostumeBases ()
		{
				var dict = new Dictionary<string, object> ();
				dict.Add ("session_id", mrdPrt.sessionId);

				mrdPrt.Connecting (dict, Enum.GetName (typeof(ProtocolType), ProtocolType.get_owncostumebases));
		}
}
